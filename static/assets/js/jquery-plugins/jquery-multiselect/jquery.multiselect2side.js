	...
	var methods = {
		init : function(options) {
			...
			return this.each(function () {
				...
				if (o.sortFunc && jQuery.isFunction(o.sortFunc)){
					window.internalSort = o.sortFunc;
				}
				...
				var customCommandDom = '';
				if (o.customCommand && jQuery.isArray(o.customCommand) && o.customCommand.length > 0){
					jQuery.each(o.customCommand,function(){
						customCommandDom +=
							"<p class='" + this.cmdclass + "' title='" + this.title + "'>" +
							this.label + "</p>";
					});
				}
				...
				if (o.search != false && o.optGroupSearch != false) {
					var ss =
						...
						o.search + "<input  type='text' class='ms2side__select_search' placeholder='search' /><a href='#'> </a>";
					...
					var	ss = o.search + "<input type='text' class='ms2side__select_search' placeholder='search'/><a href='#'> </a>";
				...
				if(o.search != false && o.optGroupSearch != false){
					el.next().children(".ms2side__select").children('.ms2side__header').css({height:"100px"})
				}else if(o.search != false || o.optGroupSearch != false){
					el.next().children(".ms2side__select").children('.ms2side__header').css({height:"52px"})
				...
				// ON 自定义被选中的列表项
				if (o.initSelectedList && jQuery.isFunction(o.initSelectedList)){
					var	selectDx = rightSel.find("option");
					if (selectDx.length != nLastAutosort) {
						selectDx = o.initSelectedList(selectDx);
						el.find("option:selected").remove();
						selectDx.each(function() {
							rightSel.append($(this).clone());
							$(this).appendTo(el).attr("selected", true);
				...
				var leftSel_dblclick = (o.leftSel_dblclick && jQuery.isFunction(o.leftSel_dblclick)) ? o.leftSel_dblclick :
					function () {
						...
						$(this).trigger('change');
						// 增加 从可选列表中移除后调用事件
						if (o.leftSel_after_remove && jQuery.isFunction(o.leftSel_after_remove)){
							o.leftSel_after_remove(leftSel,rightSel);
						}
					};
				leftSel.dblclick(leftSel_dblclick);

				// DOUBLE CLICK ON RIGHT SELECT OPTION
				var rightSel_dblclick = (o.rightSel_dblclick && jQuery.isFunction(o.rightSel_dblclick)) ? o.rightSel_dblclick :
					function () {
						$(this).find("option:selected").each(function(i, selected){
						...
						removeFilter.click();
						// 增加 从选取列表中移除后调用事件
						if (o.rightSel_after_remove && jQuery.isFunction(o.rightSel_after_remove)){
							o.rightSel_after_remove(leftSel,rightSel);
						}
					};
				rightSel.dblclick(rightSel_dblclick);
				...
				$(this).next().find('.ms2side__options').children().click(function () {
					if (!$(this).hasClass("ms2side__hide")) {
						...
						if ($(this).hasClass("RemoveAll") || $(this).hasClass("RemoveOne")){
							// 增加 从选取列表中移除后调用事件
							if (o.rightSel_after_remove && jQuery.isFunction(o.rightSel_after_remove)){
								o.rightSel_after_remove(leftSel,rightSel);
							}
						}else if ($(this).hasClass("AddOne") || $(this).hasClass("AddAll")){
							// 增加 从可选列表中移除后调用事件
							if (o.leftSel_after_remove && jQuery.isFunction(o.leftSel_after_remove)){
								o.leftSel_after_remove(leftSel,rightSel);
							}
				...
				// ON 渲染自定义的控件样式
				if (o.customCommand && jQuery.isArray(o.customCommand) && o.customCommand.length > 0){
					$(this).next().find('.ms2side__updown').children().each(function(){
						for(var p in o.customCommand){
							if ( $(this).hasClass(o.customCommand[p].cmdclass) ){
								$(this).css(o.customCommand[p].style);
							}
						}
					});
				}
				...
				$(this).next().find('.ms2side__updown').children().click(function () {
					...
					if (!$(this).hasClass("ms2side__hide")) {
						...
						// 增加 移动后调用事件
						if (o.rightSel_after_move && jQuery.isFunction(o.rightSel_after_move)){
							if ($(this).hasClass("SelSort") || $(this).hasClass("MoveUp") ||
								$(this).hasClass("MoveDown") || $(this).hasClass("MoveTop") || $(this).hasClass("MoveBottom")){
								o.rightSel_after_move(rightSel);
							}
						}
					...
					if (o.customCommand && jQuery.isArray(o.customCommand) && o.customCommand.length > 0){
						for(var p in o.customCommand){
							if ( $(this).hasClass(o.customCommand[p].cmdclass) ){
								o.customCommand[p].eventMt(rightSel);
								break;
							}
						}
					}
		...
		hideOption:function (options) {
			...
			return this.each(function () {
				...
				if (options)
					$.extend(hideOption, options);


				el.find("option[value='"+hideOption.value+"']").hide();
				// ELEMENTS
				var allSel = el.next().children(".ms2side__select").children("select");
				var	leftSel = (data.selectedPosition == 'right') ? allSel.eq(0) : allSel.eq(1);
				var	rightSel = (data.selectedPosition == 'right') ? allSel.eq(1) : allSel.eq(0);

				leftSel.children("option[value='"+hideOption.value+"']").hide();
			});
		...
		selectOption:function (options) {
			...
			return this.each(function () {
				...
				if (options)
					$.extend(selectOption, options);
				
				el.find("option[value='"+selectOption.value+"']").remove().appendTo(el).attr("selected", true);
				// ELEMENTS
				var allSel = el.next().children(".ms2side__select").children("select");
				var	leftSel = (data.selectedPosition == 'right') ? allSel.eq(0) : allSel.eq(1);
				var	rightSel = (data.selectedPosition == 'right') ? allSel.eq(1) : allSel.eq(0);
				...
				leftSel.find("option[value='"+selectOption.value+"']").remove().appendTo(rightSel);
