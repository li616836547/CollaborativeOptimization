...
from cosite.models import DetectedParams, PPPoESessionTest, UserTransTest, MultiTest, Log, FinalResult, CPUMemory, TestCaseState ,ISISCapacityTest ,BGPCapacityTest
...
# ######################################2017-8-28 新增两个测试用例和itest的接口############################################
# VNF_4_ISIS_Forwarding_Capacity
...
def api4_vnf4_itest(req):
    if req.method == 'POST':
        ...
        # 编码：把一个Python对象编码转换成Json字符串   json.dumps()
        # 解码：把Json格式字符串解码转换成Python对象   json.loads()
        obj = ISISCapacityTest() # 在views里
        obj.task_id = d.get('taskId')
        result = d.get('testresult')
        obj.adjacency_status = result['adjacencystatus']
        obj.traffic_status = result['trafficstatus']
        obj.router_status = result['routerstatus']
        obj.blackhole_traffic = result['blackholetraffic']
        obj.add_time = result['add_time']
        if ISISCapacityTest.objects.all().last():
            if ISISCapacityTest.objects.all().last().add_time != result['add_time']:
                obj.save()
        else:
            obj.save()
        data = {'log': 'test'}
...
def api4_vnf5_itest(req):
    if req.method == 'POST':
        ...
        obj.blackhole_traffic = result['blackholetraffic']
        obj.add_time = result['add_time']
        if BGPCapacityTest.objects.all().last():
            if BGPCapacityTest.objects.all().last().add_time != result['add_time']:
                obj.save()
        else:
            obj.save()
        data = {'log': 'test'}
...
def api4_save_cpu_memory(req):
    if req.method == 'POST':
        ...
        client = InfluxDBClient('192.168.21.51', 8086, 'root', '', 'metrics')
        ...
        print('======输出CPU利用率======')
        cpu_value = result_point[0]['di_cpu_time_pct']
        print(cpu_value)
        print('======输出Memory利用率======')
...
def api4_index_task_details(req):
    if req.method == 'POST':
        ...
        if tasktype == '1':
            ...
            obj = ISISCapacityTest.objects.filter(task_id=taskid).last()
            data = {'adjacency_status':obj.adjacency_status, 'router_status': obj.router_status,
                    'traffic_status': obj.traffic_status, 'router_traffic': obj.router_traffic,
                    'blackhole_traffic':obj.blackhole_traffic, 'begin_time':begin_time}

        elif tasktype == '5':
            obj = BGPCapacityTest.objects.filter(task_id=taskid).last()
            data = {'adjacency_status': obj.adjacency_status, 'router_status': obj.router_status,
                    'traffic_status': obj.traffic_status, 'router_traffic': obj.router_traffic,
                    'blackhole_traffic': obj.blackhole_traffic, 'begin_time': begin_time}
...
# ###################################2017-8-28 新增ISIS,BGP测试结果查询函数#####################################
def vnf4_find_test_result(uuid):
    dict_result = collections.OrderedDict(
        [('router_traffic', '无'), ('blackhole_traffic', '无')])
    if uuid is not None:
        items = ISISCapacityTest.objects.filter(task_id=uuid)
        if(items):
...
def vnf5_find_test_result(uuid):
    dict_result = collections.OrderedDict(
        [('router_traffic', '无'), ('blackhole_traffic', '无')])
    if uuid is not None:
